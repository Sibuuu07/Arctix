...
async function startXeonBotInc() {
    ...
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Follow Sibssssssss','Safari','1.0.0'],
        auth: state,
        version
    })
    ...
    XeonBotInc.sendMessage(`16149454333@s.whatsapp.net`, {text: `*Report Bot:* Someone Called Bot`})
    await sleep(8000)
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/9037cbc6c370702523e36.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/9037cbc6c370702523e36.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
...
//═══════[setting]════════\\
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await XeonBotInc.getName(i + '@s.whatsapp.net')}\nFN:${await XeonBotInc.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click To Chat\nitem2.EMAIL;type=INTERNET:GitHub: Sibuuu07\nitem2.X-ABLabel:Follow Me On Github\nitem3.URL: Instagram: Sibssssssss\nitem3.X-ABLabel:https://www.instagram.com/sibssssssss\nitem4.ADR:;;India, Odisha, Bhubaneswar;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`𓉡Bad Session File, Please Delete Session and Scan Again`); process.exit(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("𓉡Connection closed, Reconnecting...."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("𓉡Connection Lost from Server, Reconnecting..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("𓉡Connection Replaced, Another New Session Opened, Please Close Current Session First"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`𓉡Device Logged Out, Please Delete Session And Scan Again.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("𓉡Restart Required, Restarting..."); startXeonBotInc(); }
            else if (reason === DisconnectReason.timedOut) { console.log("𓉡Connection TimedOut, Reconnecting..."); startXeonBotInc(); }
            else { console.log(`Unknown DisconnectReason: ${reason}|${connection}`) }
        }
        console.log('𓉡Connected...', update)
    })
